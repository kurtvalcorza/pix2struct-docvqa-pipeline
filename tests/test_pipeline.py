...
def test_identity_constants():
    ...
    assert MODEL_ID == "google/pix2struct-docvqa-base"
    assert DEFAULT_WEIGHTS_DIR == REPO / "weights" / MODEL_KEY
    assert 1 <= DEFAULT_MAX_NEW_TOKENS <= MAX_NEW_TOKENS == 128
    assert MAX_PATCHES == 2048 and MAX_QUESTION_CHARS == 256
...
def _fake_pipeline(calls: list | None = None) -> Pix2StructDocVQAPipeline:
    def runner(image, question, max_new_tokens):
        if calls is not None:
            calls.append((image.mode, question, max_new_tokens))
        answer = " NW-2026-0417 " if "number" in question else "Blue Yonder Airlines"
        return {"answer": answer, "new_tokens": 12}

    return Pix2StructDocVQAPipeline(runner, "cpu", "float32", "injected")
...
def test_answer_output_fields_and_defaults():
    ...
    result = pipe.answer(Image.new("L", (400, 300)), "  What is the   invoice number? ")
    assert result["answer"] == "NW-2026-0417"  # stripped
    assert result["question"] == "What is the invoice number?"  # whitespace collapsed
    assert result["image_size"] == [400, 300]
    assert result["new_tokens"] == 12 and result["truncated"] is False
    ...
    assert calls == [("RGB", "What is the invoice number?", DEFAULT_MAX_NEW_TOKENS)]


def test_answer_reports_truncation():
    pipe = _fake_pipeline()
    result = pipe.answer(Image.new("RGB", (64, 64)), "Who is the customer?", max_new_tokens=12)
    assert result["truncated"] is True and result["answer"] == "Blue Yonder Airlines"
...
def test_answer_rejects_malformed_runner_output():
    pipe = Pix2StructDocVQAPipeline(lambda *args: {"tokens": 1}, "cpu")
