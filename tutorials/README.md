...
|---|---|---|---|---|---|---|---|
| `pix2struct_docvqa_colab.ipynb` | `E2E` | `GUIDED` | standalone; carries `pipeline.py`, `metrics.py`, and `samples.py` | deterministic 72/18/30 document-QA split; empty/majority/frozen baselines; final-two-decoder-block adaptation; held-out ANLS/exact match; safetensors export and fresh reload parity | CUDA GPU; clean Tesla T4 evidence deferred to the serial Kaggle suite | optional zip with `records.csv` and page images; gated off by default | **Candidate** — static gates pass only after this source revision; exact-blob clean-runtime evidence is pending in [`docs/release-verification.md`](../docs/release-verification.md) |
...
`tools/validate_release_assets.py`, `tools/build_notebook.py --check`, unit tests, and code-cell compilation are source checks. They are not clean-runtime execution evidence and cannot promote this Candidate.
