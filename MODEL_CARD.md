...
`google/pix2struct-docvqa-base` is the Transformers-format release of the Pix2Struct *base* model fine-tuned on DocVQA, from "Pix2Struct: Screenshot Parsing as Pretraining for Visual Language Understanding" (Lee et al., arXiv:2210.03347), converted from the T5X checkpoints by the Hugging Face team and pinned here to revision `63f6b3de436e39f75c7a486881a9c2c14a7f4e89` (the Hub's `main` on 2026-09-14). The snapshot `config.json` declares `Pix2StructForConditionalGeneration`: a 12-layer ViT-style image encoder (hidden size 768, 12 heads, feed-forward width 2048, 16×16 patches with learned row/column position embeddings over a variable-resolution input of up to 4096 patch positions) and a 12-layer T5-style text decoder (hidden size 768, 12 heads of width 64, vocabulary 50,244) — 282M parameters in the 1.13 GB float32 `model.safetensors`. The model was pretrained by parsing masked web-page screenshots into simplified HTML and fine-tuned on DocVQA (scanned industry documents with question/answer pairs). At inference the VQA processor (`Pix2StructImageProcessor`, `preprocessor_config.json`: `is_vqa` true, `max_patches` 2048) **renders the question as a text header above the page**, scales the composite to fill at most 2048 patches while preserving aspect ratio, normalises it per image and flattens it into patch tokens; the decoder then generates the answer text. Nothing is trained or adapted here. What this repository adds is packaging: `verify_snapshot` and `stage_missing_files` (manifest digest checking and fresh-clone staging), `Pix2StructDocVQAPipeline.from_pretrained` (verified local loading with `trust_remote_code=False`, refusing a snapshot whose processor is not the VQA variant), `header_font_bytes` (Pillow's bundled Aileron Regular passed to the image processor so the upstream inference-time download of `ybelkada/fonts/Arial.TTF` never happens — a deliberate, documented departure from the upstream rendering font), `answer` (input validation, whitespace-collapsed question, greedy decoding under a caller-owned `max_new_tokens`, a `truncated` flag), `normalize_answer`, `anls`, `exact_match`, and the `validate_inputs` and `evaluation_report` stage helpers.
...
The pipeline reports no accuracy measure. The answer is generated text with no score, probability or correctness signal; `new_tokens` and `truncated` describe the generation, not its quality. The repository ships DocVQA's own metric because it is what a caller would use to evaluate: `anls(prediction, golds)` — Average Normalised Levenshtein Similarity (Biten et al., 2019), `1 − lev / max(len)` over lower-cased, punctuation-stripped, whitespace-collapsed strings, maximised over the accepted answers, and scored 0 when the similarity is below `ANLS_THRESHOLD = 0.5` so near-misses are not rewarded — and `exact_match` after the same normalisation. Both need labelled question/answer pairs with matching answer conventions that the caller must supply; the DocVQA benchmark itself is registration-gated and not bundled. The public `evaluation_report(results, golds=None)` stage returns that report in machine-readable form: the mean `anls` and the `exact_match` rate over the questions plus one per-question entry (prediction, accepted answers, both scores) with the verdict `sample-sanity`, or the verdict `not-measurable` naming the labelled set that would be required when no accepted answers are supplied. The upstream paper's DocVQA ANLS (base: 72.1 on the test set, upstream-reported) is not reproduced or claimed by this pipeline.
...
This repository reports no central metric value and therefore no dispersion: the smoke run records timings, token counts and the five authored answers on one synthetic form, not accuracy. Run-to-run variability comes only from floating-point kernel selection across CPU builds and accelerators; there is no sampling and no seed to set, so a fixed input on fixed hardware is repeatable, but because decoding is autoregressive a single differing token changes the rest of the answer, and CPU and CUDA outputs need not match; the synthetic form's own bytes depend on the Pillow build's bundled font. On the synthetic form the model answered all five authored questions exactly (`anls` 1.0, `exact_match` 1.0) — one observation on one clean page, not an estimate — and answered `51759 7951` to "What is the invoice number?" on a blank 4096×4096 page, which is what an uncalibrated generator with no abstention looks like. A caller who needs an accuracy estimate must supply labelled question/answer pairs and compute ANLS over many pages or bootstrap resamples themselves; a caller who needs a confidence per answer has none from this model.
...
- **Resource use:** a 1.13 GB model and ~2.3–3.0 s per question on the reference CPU with a full encoder pass per question; a question-heavy workload scales linearly, and the CUDA path was not measured.
...
- Tutorial execution: `tutorials/pix2struct_docvqa_colab.ipynb` ran top-to-bottom in a fresh local kernel (all 8 code cells, 125.9 s including the 1.13 GB staging, same five answers as the smoke run); recorded in `docs/release-verification.md` as pre-flight, not supported-runtime evidence.
- Tests: `pytest -q -o addopts= tests` — offline, no weights required; `ruff check src tests tools` clean.
- Not executed: CUDA path, questions with non-ASCII characters (bundled-font coverage), answers longer than the default budget, any ANLS measurement against labelled pages, scans or photographs, non-Latin scripts, the upstream Arial header rendering for comparison.
