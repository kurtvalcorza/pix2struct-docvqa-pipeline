...
- E2E tutorial status: the generated 10-code-cell carrier compiles and passes static/parity checks. It has not been run locally; model execution, adaptation timings, held-out values, artifact size, and reload parity are deferred to the exact-blob Kaggle Tesla T4 run recorded in `docs/release-verification.md`.
- Tests: `pytest -q -o addopts= tests` — offline, no weights required; `ruff check src tests tools` clean.
- Not executed for the current E2E carrier: CUDA adaptation, held-out synthetic metrics, artifact reload, questions with non-ASCII characters, scans or photographs, non-Latin scripts, or the upstream Arial header rendering for comparison.
